...
#Set up contrast
sce$set_group_mode(cluster_of_interest = "dummy", ref_group = "RCV", alt_group = "CPZ")

#Run NEBULA HL
Nebula.HL_results = sce$nebula_pipeline(covs = NULL,method="HL")$res.tab

#Write out csv
out_dir <- file.path("/edgehpc/dept/compbio/projects/SingleCell/results/dev/mouse_cuprizone_DE/CRZ_vs_RCV_ST")
write.csv(Nebula.HL_results, file.path(out_dir, glue("nebula_hl_CRZ_vs_RCV_ST_{opt$annotation}.csv")), row.names = FALSE)
...
ggsave(plot = volc_plot, 
       filename = file.path(out_dir, glue("nebula_hl_CRZ_vs_RCV_ST_volcano_{opt$annotation}.png")), 
